...
@section('content')
    <div class="container">
        <div class="mb-6 grid grid-cols-1 lg:grid-cols-4 gap-5">
            ...
            <!-- TMDB Bulk Import Widget -->
            <div class="flex flex-col bg-white border shadow-sm rounded-xl dark:bg-gray-900 dark:border-gray-800 p-1 lg:col-span-4">
                <div class="px-6 py-5 flex items-center justify-between">
                    <div class="flex items-center gap-x-6">
                        <div class="flex-shrink-0 flex justify-center items-center text-gray-500 rounded-full dark:text-gray-500">
                        ...
                        <div>
                            <p class="text-sm font-medium text-gray-800 dark:text-gray-200">{{__('TMDB Bulk Import')}}</p>
                            <p class="text-xs text-gray-500 mt-1">
                                @if(config('settings.tmdb_last_sync'))
                                    {{__('Last bulk import')}}: {{ \Carbon\Carbon::parse(config('settings.tmdb_last_sync'))->diffForHumans() }}
                                @else
                                    {{__('No bulk import has been run yet')}}
                                @endif
                            </p>
                            <p class="text-xs text-amber-600 dark:text-amber-400 mt-2">
                                {{__('Optional maintenance only. TMDB search and first-visit import continue to work without running this.')}}
                            </p>
                        </div>
                    </div>
                    <div class="flex items-center gap-x-6">
                        <div class="text-center">
                            <div class="text-lg font-semibold text-gray-800 dark:text-gray-200">{{$data['movie']}}</div>
                            <div class="text-xs text-gray-500">{{__('Movies')}}</div>
                        </div>
                        <div class="text-center">
                            <div class="text-lg font-semibold text-gray-800 dark:text-gray-200">{{$data['tv']}}</div>
                            <div class="text-xs text-gray-500">{{__('TV Shows')}}</div>
                        </div>
                        <form action="{{route('admin.tmdb.syncnow')}}" method="POST">
                            @csrf
                            <button type="submit" class="py-2 px-4 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-primary-500 text-white hover:bg-primary-600 transition">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182M21.015 4.356v4.992" />
                                </svg>
                                {{__('Bulk Import (Manual)')}}
                            </button>
                        </form>
                    </div>
                </div>
            </div>
            <!-- End TMDB Bulk Import Widget -->
