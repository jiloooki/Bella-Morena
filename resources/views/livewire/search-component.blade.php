<div>
    ...
    <div id="search-modal"
        ...
        <div class="bg-white overflow-auto max-w-3xl w-full max-h-full rounded-xl px-2 py-1"
            ...
            @if(count($posts) > 0)
                <div class="mb-1 last:mb-0 py-2 px-2">
            ...
            @if(count($tmdbResults) > 0)
                <div class="mb-1 last:mb-0 py-2 px-2 border-t border-gray-100">
                    <div class="text-xs text-gray-400 px-2 mb-2 flex items-center gap-2">
                        <span>{{__('More from TMDB')}}</span>
                    </div>
                    <ul class="text-sm">
                        @foreach($tmdbResults as $tmdb)
                            <li>
                                <a class="text-sm text-gray-600 p-2 dark:text-gray-200 flex space-x-6 group items-center"
                                   href="{{route($tmdb['type'] === 'movie' ? 'movie' : 'tv', 'tmdb-' . $tmdb['tmdb_id'])}}">
                                    <div class="aspect-poster bg-gray-100 rounded-md w-14 overflow-hidden relative">
                                        @if($tmdb['image'])
                                    ...
                                    <div class="flex-1">
                                        <div class="font-medium group-hover:underline mb-1 text-gray-700 line-clamp-1 flex items-center gap-2">
                                            {{$tmdb['title']}}
                                            <span class="text-xxs bg-primary-100 text-primary-600 rounded px-1.5 py-0.5 font-normal">
                                                {{$tmdb['type'] === 'movie' ? __('Movie') : __('TV Show')}}
                                            </span>
                                        </div>
                                        <div class="text-xs text-gray-400 dark:text-gray-500 line-clamp-2">{{Str::limit($tmdb['overview'], 80)}}</div>
                                    </div>
                                </a>
                            </li>
                        @endforeach
            ...
            @if(count($posts) > 0 || count($tmdbResults) > 0)
                <div class="border-t text-center border-gray-100 mt-1">
